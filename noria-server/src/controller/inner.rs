...
impl DomainReplies {
    ...
    fn wait_for_user_rows(&mut self, d: &DomainHandle) -> HashMap<String, Vec<Vec<u8>>> {
...
impl ControllerInner {
    ...
    fn import_user_shard(
        ...
        for (domain, user_rows) in domain_imported {
            ...
            let domain_handle = self.domains.get_mut(&domain).unwrap();
            match domain_handle
                .send_to_healthy(box Packet::ImportUserRows { user_rows }, &self.workers)
            {
                Ok(_) => (),
                Err(e) => match e {
                    SendError::IoError(ref ioe) => {
